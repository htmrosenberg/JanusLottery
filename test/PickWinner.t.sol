...
contract PickWinnerTest is Test, CodeConstants {
    ...
    function testPickNoTicketWinner() public {
        ...
        uint256 fee_prize =
            ((janusLottery.getTotalTicketsSale() + janusLottery.getJackpot()) * janusLottery.getFeePromille()) / 1000;
        uint256 funder_prize = (
            (janusLottery.getJackpot() + janusLottery.getTotalTicketsSale()) * (1000 - janusLottery.getFeePromille())
        ) / 1000;
        ...
        VRFCoordinatorV2_5Mock(networkConfig.vrfCoordinatorV2_5).fulfillRandomWords(1, address(janusLottery));
